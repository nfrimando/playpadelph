...
export default function PointForm({ point, players, events, onClose, onSaved }: PointFormProps) {
  const [form, setForm] = useState({
    player_id: point?.player_id?.toString() ?? '',
    event_id: point?.event_id?.toString() ?? '',
      ...
      <div className="bg-white border border-line rounded-[3px] w-full max-w-[440px] mx-4 shadow-[0_8px_40px_rgba(0,0,0,.18)]">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <select value={form.player_id} onChange={set('player_id')} required className={inputClass}>
          ...
          <div>
            ...
            <select value={form.event_id} onChange={set('event_id')} required className={inputClass}>
          ...
          <div>
            ...
            <input type="number" min="0" value={form.points} onChange={set('points')} required
              className={inputClass} placeholder="e.g. 1000" />
          </div>
          ...
          <div>
            ...
            <input type="date" value={form.date_added} onChange={set('date_added')} required
