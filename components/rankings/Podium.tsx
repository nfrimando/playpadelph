      ...
      <div className="rounded-[3px] p-7 pb-6 text-center bg-green shadow-[0_14px_40px_rgba(26,92,56,.26)] -translate-y-2.5">
        ...
        {player.id != null ? (
          <Link
            ...
            className="font-display text-lg font-semibold leading-tight mb-0.5 text-oat hover:opacity-75 transition-opacity block"
          >
            {player.name}
          </Link>
        ) : (
          <div className="font-display text-lg font-semibold leading-tight mb-0.5 text-oat">
            {player.name}
          </div>
        )}
        {player.nationality && (
          <span className="text-[9px] tracking-[2px] uppercase block mb-[18px] text-oat/42">
            {player.nationality}
          </span>
        )}
        <span
          className={`font-display text-[42px] font-bold block ${pointColors[0]}`}
        >
          {player.points.toLocaleString()}
        </span>
        <span className="text-[8.5px] tracking-[2.5px] uppercase block mt-0.5 text-oat/35">
          pts
        </span>
      </div>
    );
  ...
  return (
    <div className="rounded-[3px] p-7 pb-6 text-center bg-white border border-line shadow-[0_3px_12px_rgba(0,0,0,.06)]">
      ...
      {player.id != null ? (
        <Link
          href={`/players/${player.id}`}
          className="font-display text-lg font-semibold leading-tight mb-0.5 text-ink hover:text-green transition-colors block"
        >
          {player.name}
        </Link>
      ) : (
        <div className="font-display text-lg font-semibold leading-tight mb-0.5 text-ink">
          {player.name}
        </div>
      )}
      {player.nationality && (
        <span className="text-[9px] tracking-[2px] uppercase block mb-[18px] text-mid">
          {player.nationality}
        </span>
      )}
      <span
        className={`font-display text-[42px] font-bold block ${pointColors[position]}`}
      >
        {player.points.toLocaleString()}
      </span>
      <span className="text-[8.5px] tracking-[2.5px] uppercase block mt-0.5 text-mid">
        pts
      </span>
    </div>
  );
